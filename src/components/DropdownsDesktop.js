...
const DropdownsDesktop = () => {
  ...
  if (mode === winterMode || mode === summerMode) {
  ...
  const switchMode = (e) => {
    if (mode === summerMode) {
      body.classList.replace(summerMode, winterMode);
      e.target.classList.remove(clickedState);
      localStorage.setItem("mode", "winter");
      mode = winterMode;
    } else {
      body.classList.replace(winterMode, summerMode);
    ...
    <div className="flex items-row justify-end">
      <div className="relative inline-block text-left">
        <div>
          <button
            type="button"
            className="flex justify-center items-center w-14 rounded-full border
     ...
     focus:outline-none focus:ring-2 focus:ring-offset-2
      focus:ring-offset-gray-100 focus:{color-winter-primary}"
        ...
        {showMenu /* It's like if/else */ && (
          <div
            className="origin-top-right absolute right-0 mt-2 w-56 rounded-lg shadow-md shadow-color-font-dark  bg-slate-300 ring-1 ring-black ring-opacity-5 focus:outline-none"
            role="menu"
          ...
          >
            <div className="py-1" role="none">
              <button
                type="submit"
                className="text-black block w-full text-left px-4 py-2 text-sm hover:bg-color-winter-header hover:text-color-font-light rounded-lg"
                role="menuitem"
                tabindex="-1"
                id="menu-item-1"
              >
              ...
              <a
                href="#"
                className="text-black block px-4 py-2 text-sm hover:bg-color-winter-header hover:text-color-font-light rounded-lg"
                role="menuitem"
                tabindex="-1"
                id="menu-item-0"
              >
              ...
              <button
                type="submit"
                className={mode === "winter" ? clickedState : ""}
                role="menuitem"
                tabindex="-1"
                id="menu-item-1"
              ...
              <button
                type="submit"
                // className="text-black block w-full text-left px-4 py-2 text-sm hover:bg-color-winter-header hover:text-color-font-light rounded-lg"
                className={mode === "summer" ? clickedState : ""}
                role="menuitem"
                tabindex="-1"
                id="menu-item-2"
              >
